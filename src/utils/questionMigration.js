	...
	try {
		...
		for (const question of questions) {
			...
			if (
				...
				question.answer_options[0]?.text
			) {
				console.log(
					`⏭️  Question already migrated: ${question.question_text?.substring(
						0,
						50
					)}...`
				);
			...
			if (
				...
				const enhancedAnswerOptions = question.answer_options.map(
					(option, index) => ({
				...
				if (updateError) {
					console.error(
						`❌ Failed to migrate question ${question.id}:`,
						updateError
					);
				} else {
					console.log(
						`✅ Migrated: ${question.question_text?.substring(0, 50)}...`
					);
					migratedCount++;
				}
			}
			// Handle old format with 'options' column (string array)
				...
				console.log(
					`⚠️  Skipping question (unsupported format): ${question.question_text?.substring(
						0,
						50
					)}...`
				);
	...
	try {
		...
		if (question.options && question.correct_answer !== null) {
			...
			console.log(
				...
				JSON.stringify(enhancedQuestionData, null, 2)
			);
		}
